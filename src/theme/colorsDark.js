const colors = {
  primary: '#4285F4',
  info: '#05bfec',
  success: '#2DA94F',
  danger: '#ED412D',
  warning: '#FDBD00',
  transparent: 'transparent',

  // caret color
  caret: '#00d800',
  caretHighlight: '#01AA00',

  // main background
  background: '#ECEFF1',

  // banner
  bannerBackground: '#ECEFF1',

  // activityIndicator color
  activityIndicator: '#1A314D',

  // text colors
  textMuted: '#80847A',

  textDefaultShadow: '#1A314D',
  textDefault: '#2B425E',
  textDefaultLighten: '#435770',

  inputText: '#2B425E',
  inputTextDisabled: '#AAA',
  inputTextBackground: '#FFF',
  inputTextBorder: '#DDD',

  // component colors
  navigationBackground: '#111111',
  navigationShadow: '#b93400',
  navigationText: '#FFF',

  // drawer colors
  drawerBackground: '#FFF',
  drawerBackgroundHighlight: '#111111',
  drawerShadow: '#AAA',
  drawerBorder: '#EAEAEA',
  drawerText: '#1A314D',
  drawerTextHighlight: '#FFF',
  drawerTextMuted: '#80847A',
  drawerTextMutedHighlight: '#EAEAEA',

  // breedcrumb
  breedCrumbBackground: '#111111',
  breedCrumbBorder: '#111111',
  breedCrumbText: '#FFFFFF',
  breedCrumbTextMuted: '#FFFFFF',

  // menuItem
  menuItemBackground: '#FFF',
  menuItemBorder: '#DDD',
  menuItemText: '#1A314D',
  menuItemTextMuted: '#80847A',

  menuItemBackgroundHighlight: '#111111',
  menuItemBorderHighlight: '#BCC0C3',
  menuItemTextHighlight: '#FFFFFF',
  menuItemTextMutedHighlight: '#80847A',

  companyItemBorder: '#111111',

  // section
  sectionBackground: '#FFF',

  // barChart
  chartAxis: '#34495E',
  chartLine: '#2980B9',
  chartGrid: '#888',

  // Score Card
  scoreCardBackground: '#FFF',
  scoreCardBorder: '#111111',

  scoreCardDateBackgroundHighlight: '#34495E',
  scoreCardDateBackground: '#94a6be',
  scoreCardDateText: '#FFF',

  // Ball Color
  ballText: '#FFF',
  ballBackground: '#424446', // '#94a6be',

  todayBallBackground: '#34495E',
  todayBallText: '#FFF',

  bonusBallText: '#FFF',
  bonusBallBackground: '#0d4880', // '#05bfec',

  matchBallText: '#FFF',
  matchBallBackground: '#ED412D',

  wrongBallText: '#FFF',
  wrongBallBackground: '#4285F4',

  // table
  tableHeaderBackground: '#ECEFF1',
  tableHeaderText: '#2B425E',
  tableHeaderBorder: '#BCC0C3',
  tableItemBackground: '#FFF',
  tableItemText: '#1A314D'
};

export default colors;
